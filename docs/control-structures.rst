...
The ``unchecked`` block can be used everywhere where a block can be used.
...
.. note::
    If you use ``unchecked { _; }`` inside a modifier, this will **not** cause the
    function to use wrapping arithmetic.

An overflow / underflow is the situation that the resulting value of an arithmetic operation,
when executed on an unrestricted integer, falls outside the value range of the type of the result.

The following operators will cause a failing assertion on overflow / underflow
and will wrap without error if used inside an unchecked block:
...
.. note::
    The second statement in ``int x = type(int).min; -x;`` will result in an underflow
